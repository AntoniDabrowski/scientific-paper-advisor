...
inverse_bins_2_grams = pickle.load(open(f'inverse_bins_2_gram.pickle', 'rb'))
inverse_bins_3_grams = pickle.load(open(f'inverse_bins_3_gram.pickle', 'rb'))
...
def POS_bigrams_frequency(text):
    popular_bigram_tags = ['NNP-NNS', 'VBD-JJ', 'VBD-RB', 'NNS-VBD', 'JJ-NNP', 'CD-JJ', 'CC-CD', 'VBD-DT', 'CD-NN', 'JJ-CD', 'VBP-IN', 'JJ-PNC', 'NN-NNP', 'VBD-IN', 'RB-VBN', 'JJ-JJ', 'VBD-VBN', 'PRP-VBD', 'NN-CD', 'NNP-VBD', 'NNS-PNC', 'IN-CD', 'NN-VBD', 'NNS-VBN', 'NNP-CC']
    tags_occurrences = dd(int)
    total = 1
    ...
    for i in range(1,len(text_tags)):
        ...
        if bigram_tags in popular_bigram_tags:
            ...
            total += 1
    return [tags_occurrences[tag] / total for tag in popular_bigram_tags], popular_bigram_tags
...
def CSS_vector(text, n=2, bins_count=10):
    ...
    return [np.log(bins_evaluation[i]) for i in range(1,bins_count+1)], [f'bin{i}' for i in range(1,bins_count+1)]
